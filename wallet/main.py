...
async def get_wallet_info(wallet_uuid: str) -> dict:
    try:
        ...
        raise HTTPException(
            status_code=404, detail=f"Не удалось получить информацию о кошельке: {e}"
        )
...
async def deposit_to_wallet(wallet_uuid: str, amount: Amount):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка при увеличении баланса: {e}"
        )  # 500 - потому что,
...
async def withdraw_from_wallet(wallet_uuid: str, amount: Amount):
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Ошибка при уменьшении баланса: {e}"
        )  # а тут скорее проблема
