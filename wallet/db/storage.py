        ...
        async with self.async_session() as session:
            ...
            if not result:
                await self.create_wallet(
                    wallet_uuid, 0
                )  # создаем кошелек в любом случае
                raise Exception(
                    f"Кошелек {wallet_uuid} имеет нулевой баланс. Не хватает средств для вывода"
                )
    ...
    async def check_tables_exist(self) -> bool:
        async with self.engine.connect() as conn:
            result_for_wallet_table = await conn.execute(
                text("SELECT to_regclass('public.wallet')")
            )  # используем стандартную схему PostgreSQL - public
